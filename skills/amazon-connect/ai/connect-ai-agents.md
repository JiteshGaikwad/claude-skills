...
The orchestrator agent invokes tools to perform actions. Three tool types are supported:
...
Return to Control tools hand execution back to the Connect flow for processing. Two built-in types plus custom:
...
**Custom Return to Control tools** allow structured data to be passed back to the flow with a defined JSON input schema. Example -- custom escalation tool:
...
AI agents require a security profile that defines:

- Which tools the agent can access
- Which knowledge bases it can query
- Rate limits and usage quotas
- Logging and audit configuration

### Setup Sequence

1. **Create orchestrator agent** -- define the agent with its base configuration
2. **Create security profile** -- assign permissions and tool access
3. **Add tools** -- attach MCP tools, Return to Control tools, and/or Constant tools
4. **Configure orchestration prompt** -- customize the system prompt that guides agent behavior
5. **Set as default** -- designate the orchestrator as the default for the instance
6. **Create conversational AI bot** -- create a Lex bot that serves as the conversational front-end
7. **Build flow** -- wire the bot into a Connect contact flow

### Handling Return to Control in Flows

When a Return to Control tool fires, the flow receives control back. Use the **Check contact attributes** block to inspect the Lex session attribute `"Tool"` to determine which tool triggered the return:
...
Model availability varies by AWS region. Common models include:

- **Claude Sonnet 4.5** -- high capability, available in us-east-1, us-west-2, eu-west-2, ap-southeast-2
- **Amazon Nova Pro** -- balanced performance/cost, broadly available
- **Claude Haiku** -- fastest response times, lowest cost, broadly available
...
- Example: "competitor comparisons", "investment advice", "medical diagnosis"
...
|---|------------|-------------|
| 1 | **Orchestration** | Top-level orchestrator for agentic self-service |
| 2 | **Answer Recommendation** | Suggests answers to agents during live conversations |
| 3 | **Manual Search** | Powers agent-initiated knowledge base searches |
| 4 | **Self Service** | Handles autonomous customer self-service interactions |
| 5 | **Email Response** | Generates email replies to customer inquiries |
| 6 | **Email Overview** | Summarizes email threads for agent context |
| 7 | **Email Generative Answer** | Generates answer content for email from KB |
| 8 | **Note Taking** | Creates structured notes from conversations |
| 9 | **Agent Assistance** | Provides real-time suggestions during agent conversations |
| 10 | **Case Summarization** | Summarizes multi-interaction customer cases |
